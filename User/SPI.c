...
void DMA2_Stream0_IRQHandler(void)
{
	...
	if (DMA_GetITStatus(DMA2_Stream0, DMA_IT_TCIF0) == SET)
	{
		...
		if((buffer[2] < 0x7F || buffer[2] > 0x83) && (buffer[4] < 0x7C || buffer[4] > 0xC1) && (buffer[6] < 0xBC || buffer[6] > 0xC0))
		{
...
void SPI_ini(void)
{
	...
	GPIO_Init_ACC.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;
	GPIO_Init_ACC.GPIO_Mode = GPIO_Mode_AF;
	GPIO_Init_ACC.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init_ACC.GPIO_OType = GPIO_OType_PP;
	GPIO_Init_ACC.GPIO_PuPd = GPIO_PuPd_NOPULL;
	
	GPIO_Init(GPIOA, &GPIO_Init_ACC);
	...
	GPIO_Init_ACC.GPIO_Pin = GPIO_Pin_3;
	GPIO_Init_ACC.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_Init_ACC.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init_ACC.GPIO_OType = GPIO_OType_PP;
	GPIO_Init_ACC.GPIO_PuPd = GPIO_PuPd_NOPULL;
	
	GPIO_Init(GPIOE, &GPIO_Init_ACC);
